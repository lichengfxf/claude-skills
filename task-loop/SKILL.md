...
### 每个任务循环开始前
**必须清空当前会话的上下文**，以避免 Token 消耗过大和保持处理效率。

清空上下文的操作：
```markdown
在开始处理新任务之前，执行以下操作：
1. 读取当前任务信息（任务编号、描述、状态）
2. 读取相关文件（TASKS.md、CONTEXT.md、对应的 .review 文件）
3. 告诉用户："清空上下文，仅保留当前任务相关文件"
4. 继续执行任务
```
...
2025-02-24 10:00:03 | INFO  | 清空上下文，仅保留当前任务信息
2025-02-24 10:00:04 | INFO  | 开始处理任务 01-001 [TODO]
2025-02-24 10:00:05 | TASK  | 执行 task-exec：01-001 初始化项目结构
2025-02-24 10:15:30 | TASK  | 完成 task-exec，状态：[WIP] → [DONE]
2025-02-24 10:15:31 | TASK  | 执行 task-review：01-001
...
2025-02-24 10:20:18 | INFO  | 清空上下文，仅保留当前任务信息
2025-02-24 10:20:19 | INFO  | 开始处理任务 01-002 [TODO]
...
```
...
```markdown
1. 检查 TASKS.md 是否存在
2. 检查 CONTEXT.md 是否存在（可选）
3. 创建/清空 task-loop-status.md（进度报告文件）
4. 创建/清空 task-loop-log.txt（运行日志文件）
5. 写入初始日志：
   ```
   {时间戳} | INFO | 启动自动化开发循环
   {时间戳} | INFO | 初始化进度报告和日志文件
   ```
6. 读取并分析 TASKS.md，统计任务数量和状态
7. 写入初始进度报告到 task-loop-status.md
```
...
循环会：
1. 读取当前状态
2. 追加日志到 task-loop-log.txt（不清空）
3. 更新 task-loop-status.md
4. 从第一个未完成的任务继续
...
步骤 1：清空上下文
─────────────────────────────────────────
操作：清空会话上下文，仅保留当前任务相关文件
日志：{时间戳} | INFO | 清空上下文，仅保留当前任务信息
