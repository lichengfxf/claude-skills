...
**场景 C：修复后再次被拒绝（从 [REJECTED] 再次修复）**
- 如果 review 后再次被拒绝，需要再次修复
- 修复报告文件名：`task-{编号}.fix.2`（第二次修复）

如果需要，你可以从 CONTEXT.md 文件获取项目整体信息
你可以参考对应的评审报告文件（如 task-01-01-001.review）中的详细评审意见
...
1. **分析评审意见**：
   - 阅读任务下方的评审意见
   - 查看详细的评审报告文件（首次评审为 `.review`，第二次评审为 `.review.2`）
   - 理解需要修复的问题
...
3. **完成修复**：
   - 确保问题已解决
   - 将 [FIXING] 改为 [FIXED]
   - 首次修复：生成修复报告到 `task-{编号}.fix` 文件
   - 第二次修复：生成修复报告到 `task-{编号}.fix.2` 文件
   - 提交修复代码
...
### 修复前准备
1. 识别任务编号（如 01-01-001）
2. 判断任务状态：
   - 如果是 [REJECTED]：准备开始修复
   - 如果是 [FIXING]：准备恢复中断的修复
3. 阅读任务下方的评审意见
4. 查看对应的评审报告文件（如 task-01-01-001.review）
5. 理解需要修复的问题
...
1. 判断修复次数：
   - 如果存在 `task-{编号}.fix` 文件但不存在 `.fix.2`，则是第二次修复
   - 如果存在 `task-{编号}.fix.2`，则是第三次修复（以此类推）
2. 将任务状态从 [REJECTED] 改为 [FIXING]（开始修复）
3. 根据评审意见进行修复
4. 自测验证修复效果
5. 将任务状态从 [FIXING] 改为 [FIXED]（修复完成）
6. 生成修复报告：
   - 首次修复：生成到 `task-{编号}.fix` 文件
   - 第二次修复：生成到 `task-{编号}.fix.2` 文件
   - 以此类推：`.fix.3`、`.fix.4` 等
7. 清理任务下方的评审意见（可选）
...
6. 生成修复报告到 `task-{编号}.fix` 文件（保持原有逻辑）
7. 清理任务下方的评审意见（可选）

### 修复完成后
```markdown
# 修复前
- [REJECTED] 01-02-001 [依赖: 01-01-002] 实现登录功能 → POST /login 返回 token
  - 评审意见：缺少输入验证
  - 评审意见：密码未加密

# 修复中
- [FIXING] 01-02-001 [依赖: 01-01-002] 实现登录功能 → POST /login 返回 token
  - 评审意见：缺少输入验证
  - 评审意见：密码未加密

# 修复后（保留评审意见）
- [FIXED] 01-02-001 [依赖: 01-01-002] 实现登录功能 → POST /login 返回 token
  - 评审意见：缺少输入验证
  - 评审意见：密码未加密
...
- [ ] 没有引入新的问题
- [ ] 任务状态已改为 [FIXED]
- [ ] **修复报告已生成（首次为 task-{编号}.fix，第二次为 task-{编号}.fix.2）**

## 完成时

### 必须完成的工作
1. 将任务状态改为 [FIXED]
2. 生成修复报告（首次为 `task-{编号}.fix`，第二次为 `task-{编号}.fix.2`）
3. 提交代码："fix: 01-02-001 [任务描述] - 修复评审意见"

### 修复后的状态
修复完成后，任务将重新进入评审流程（[FIXED] → [REVIEW]）

评审时可以参考：
- 首次修复报告文件：`task-{编号}.fix`
- 第二次修复报告文件：`task-{编号}.fix.2`
- 原始评审报告文件：`task-{编号}.review`
- 第二次评审报告文件：`task-{编号}.review.2`
