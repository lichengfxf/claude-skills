...
你可以参考对应的评审报告文件（如 task-001.review）中的详细评审意见
...
1. 识别任务编号（如 #001）
2. 阅读任务下方的评审意见
3. 查看对应的评审报告文件（如 task-001.review）
4. 理解需要修复的问题
...
# 修复中
- [FIXING] #001 [依赖: XXX] 实现登录功能 → POST /login 返回 token
  - 评审意见：缺少输入验证
  - 评审意见：密码未加密

# 修复后（保留评审意见）
- [FIXED] #001 [依赖: XXX] 实现登录功能 → POST /login 返回 token
  - 评审意见：缺少输入验证
  - 评审意见：密码未加密
