...
## 任务状态管理工作流
任务生命周期：`待开发 → 开发中 → 开发完成 → 评审中 → 评审通过/拒绝`
...
- [👀] [依赖: 登录 API] 实现登出功能 → DELETE /logout 返回 200
- [✓] [依赖: 登录 API] 添加 token 刷新机制 → 刷新后获得新 token
- [✗] [依赖: 登录 API] 实现密码重置 → POST /reset-password 返回 200
  - 评审意见：需要添加验证码校验
- [🔧] [依赖: 登录 API] 实现密码重置 → 添加验证码校验功能
```
...
- [✗] [依赖: 认证] 实现角色管理 API → POST /roles 返回 201
  - 评审意见：需要添加权限验证逻辑
- [🔧] [依赖: 认证] 实现角色管理 API → 添加权限验证逻辑
```
