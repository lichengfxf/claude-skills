...
### 阶段 2：核心功能（依赖阶段 1）
- [TODO] 01-003 [依赖: 测试框架] 实现 API 基类 → GET /api/health 返回 200
- [WIP] 01-004 [依赖: API 基类] 实现用户模型 → User 模型通过验证测试
...
### 核心功能
- [TODO] 01-004 [依赖: 01-002] 实现用户模型 → User 类型定义完整
- [WIP] 01-005 [依赖: 01-004] 实现用户注册 API → POST /users 返回 201
- [TODO] 01-006 [依赖: 01-005] 实现用户登录 API → POST /login 返回 token
