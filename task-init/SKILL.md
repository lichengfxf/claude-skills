...
```markdown
### 阶段 1：基础设施（无依赖）
- [TODO] Setup: 初始化项目结构 → package.json 和目录结构存在
- [TODO] Setup: 配置测试框架 → npm test 成功运行

### 阶段 2：核心功能（依赖阶段 1）
- [TODO] [依赖: 测试框架] 实现 API 基类 → GET /api/health 返回 200
- [WIP] [依赖: API 基类] 实现用户模型 → User 模型通过验证测试

### 阶段 3：业务逻辑（依赖阶段 2）
- [DONE] [依赖: 用户模型] 实现登录 API → POST /api/login 成功返回 token
- [REVIEW] [依赖: 登录 API] 实现登出功能 → DELETE /logout 返回 200
- [APPROVED] [依赖: 登录 API] 添加 token 刷新机制 → 刷新后获得新 token
- [REJECTED] [依赖: 登录 API] 实现密码重置 → POST /reset-password 返回 200
  - 评审意见：需要添加验证码校验
- [FIXING] [依赖: 登录 API] 实现密码重置 → 添加验证码校验功能
```
...
### 基础设施
- [TODO] 初始化项目结构 → package.json 和目录结构存在
- [WIP] 配置 TypeScript → tsc --noEmit 成功
- [DONE] 配置测试框架 → npm test 成功运行

### 核心功能
- [TODO] [依赖: TypeScript] 实现用户模型 → User 类型定义完整
- [WIP] [依赖: 用户模型] 实现用户注册 API → POST /users 返回 201
- [TODO] [依赖: 用户注册] 实现用户登录 API → POST /login 返回 token

## 🎯 里程碑 2：增强
- [DONE] [依赖: 登录] 添加 JWT 认证中间件 → 认证请求正常处理
- [REVIEW] [依赖: 认证] 实现权限控制 → 未授权请求返回 401
- [REJECTED] [依赖: 认证] 实现角色管理 API → POST /roles 返回 201
  - 评审意见：需要添加权限验证逻辑
- [FIXING] [依赖: 认证] 实现角色管理 API → 添加权限验证逻辑
```
